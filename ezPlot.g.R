ezPlot.g=function (data, dv, sid, within = NULL, between = NULL, x, do_lines = TRUE, 
    do_bars = TRUE, bar_width = NULL, bar_size = NULL, split = NULL, 
    row = NULL, col = NULL, to_numeric = NULL, x_lab = NULL, 
    y_lab = NULL, split_lab = NULL, levels = NULL) 
{
    if (!is.null(levels)) {
        for (i in 1:length(levels)) {
            this_iv = names(levels)[i]
            data[, names(data) == this_iv] = factor(data[, names(data) == 
                this_iv])
            if ("new_order" %in% names(levels[[i]])) {
                data[, names(data) == this_iv] = factor(data[, 
                  names(data) == this_iv], levels = levels[[i]]$new_order)
            }
    ...
    for (i in to_numeric) {
        data[, names(data) == i] = as.numeric(as.character(data[, 
            names(data) == i]))
    }
    ...
    p = ggplot(data = data, aes(y = Mean, x = x))
    if (!is.null(split)) {
        p = p + geom_point(aes(colour = split, shape = split))
        p = p + opts(legend.background = theme_rect(colour = "transparent", 
            fill = "transparent"))
        if (!is.null(split_lab)) {
            p = p + labs(colour = split_lab, shape = split_lab)
        }
        if (do_lines) {
            p = p + geom_line(aes(colour = split, linetype = split, 
                x = as.numeric(x)))
            if (!is.null(split_lab)) {
        ...
        if (do_bars) {
            p = p + geom_errorbar(aes(colour = split, ymin = ymin, 
                ymax = ymax), linetype = 1, legend = FALSE, width = bar_width)
        }
    }
    else {
        p = p + geom_point()
        ...
        if (do_bars) {
            p = p + geom_errorbar(aes(ymin = ymin, ymax = ymax), 
                linetype = 1, legend = FALSE, width = bar_width)
        }
    }
    if (!is.null(row)) {
        if (!is.null(col)) {
