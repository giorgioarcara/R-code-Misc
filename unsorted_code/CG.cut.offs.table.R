CG.cut.offs.table<-function(mod, vars, values, upper=FALSE){
...
"'mod' has length: ", n_pred, "\n",
  ...
  if (length(vars==1)){
    ...
    t.crit=qt(p=0.05, df=df) #nota che uso 0.05
    
    if (upper==FALSE){
      t.crit=qt(p=0.05, df=df) #nota che uso 0.05
    }
    if (upper==TRUE){
      t.crit=-qt(p=0.05, df=df)
    }
    
    
    # questa formula che segue non è altro che la formula (3) del paper di Crawford e Garthwaite, girata. 
    # Nota che l'output di quella formula è il t osservato
  ...
  if (length(vars)>1){
    ...
    if (upper==TRUE){
    t.crit=qt(p=0.95, df=df) # notice, it's the same of using t.crit=-qt(p=0.95, df=f)
    }
    
    # questa formula che segue non è altro che la formula (3) del paper di Crawford e Garthwaite, girata. Nota che l'output di quella formula è il t osservato
    Y_obs_crit=(t.crit*Sn_1)+Y_pred 
