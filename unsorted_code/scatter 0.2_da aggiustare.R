...
scatter=function(dep1, dep2, fac, dat){
...
if (length(levels(factor(dat[,fac])))>length(color.list)){
	...
	for (i in 1:length(levels(as.factor(as.character(dat[,nfac]))))) {
		if (i==1){
		plot(dat[dat[,nfac]==paste(levels(as.factor(as.character(dat[,nfac])))[i]),ndep1], 	dat[dat[,nfac]==paste(levels(as.factor(as.character(dat[,nfac])))[i]),ndep2], col=paste(color.list)[i], 	main=paste(toupper(names(dat)[ndep1]),"X", toupper(names(dat)[ndep2])), xlab=paste(names(dat)[ndep1]), 	ylab=paste(names(dat)[ndep2]), xlim=range(na.omit(dat[,ndep1])),ylim=range(na.omit(dat[,ndep2])), pty="s",pch=21,bg=paste(color.list)[i])
	lines(lowess(dat[dat[,nfac]==paste(levels(as.factor(as.character(dat[,nfac])))[i]),ndep1],	dat[dat[,nfac]==paste(levels(as.factor(as.character(dat[,nfac])))[i]),ndep2], na.rm=TRUE)$fitted, col=paste(color.list)[i])
	par(new=TRUE)
	}
	if (i!=1){
	plot(dat[dat[,nfac]==paste(levels(as.factor(as.character(dat[,nfac])))[i]),ndep1], 	dat[dat[,nfac]==paste(levels(as.factor(as.character(dat[,nfac])))[i]),ndep2], col=paste(color.list)	[i],frame.plot=FALSE, axes=FALSE,main="", xlab="", ylab="", pty="s", pch=21,bg=paste(color.list)[i])
	lines(lowess(dat[dat[,nfac]==paste(levels(as.factor(as.character(dat[,nfac])))[i]),ndep1], 	dat[dat[,nfac]==paste(levels(as.factor(as.character(dat[,nfac])))[i]),ndep2], na.rm=TRUE)$fitted, col=paste(color.list)[i])
	par(new=TRUE)}
	}
