...
source("functions/grandaverage.R")
source("functions/scalp.R")

load("data/ERPsets.RData") # load data
  ...
  fluidRow(
    column(12,
           ...
           sidebarLayout(
             ...
             mainPanel(
               plotOutput("Shiny_Topoplot", hover="plot_hover") 
             )
           )
    ),
    fluidRow(
...
server <- function(input, output) {
  ...
  }) # end output Shiny_topoplot
  
  
  output$Shiny_scalp= renderPlot({
    
    scalp(list(word), layout=1, startmsec=my_startmsec, endmsec=my_endmsec, ylim=6)
