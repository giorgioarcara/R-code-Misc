...
#filter=TRUE . Se TRUE allora vengono restituiti solo i risultati significativi. Altrimenti vengono restituiti tutti i risultati, ma laddove c'era violazione di sfericità accanto è riportato anche la correzione GG e HF.
# digit imposta il numero di numeri significativi dopo la virgola dei p-value.


ezresults=function(mod, filter=TRUE, digit=2)
	{
	if(length(mod)==1)
	{
	anovares=mod[[1]]
