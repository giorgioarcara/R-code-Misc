...
ezANOVA.nout = function(data.name=NULL, dv=NULL, wid=NULL, between=NULL, within=NULL, out, n, p.crit=0.05, plot=FALSE, plot.par=NULL, cex.x=1, plot.vars="all", ...) {
	
	
	
	data=eval(parse(file="", text=data.name))
	...
	for (i in 1:dim(out.s)[2]){
		...
		res=eval(parse(file="", text=paste("ezANOVA(data=", data.name, ", dv=.(", dv, "), wid=.(", wid, "), ", "within=.(", within, "))", sep="")))  #
		}
		
		if (is.null(within)&!(is.null(between))){
		res=eval(parse(file="", text=paste("ezANOVA(data=", data.name, ", dv=.(", dv, "), wid=.(", wid, "),", "between=.(", between, "))", sep="")))  #
		}
		
		if (!(is.null(within))&!(is.null(between))){
		res=eval(parse(file="", text=paste("ezANOVA(data=", data.name, ", dv=.(", dv, "), wid=.(", wid, "), ", "within=.(", within,"), ", "between=.(", between, "))", sep=""))) 
		pr=paste("ezANOVA(data=", data.name, ", dv=.(", dv, "), wid=.(", wid, "), ", "within=.(", within,"), ", "between=.(", between, "))", sep="")
		cat(pr)
		}
